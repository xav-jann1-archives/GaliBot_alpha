...
const ESC = String.fromCharCode(27);   // Exit char when writing a text message
...
constructor(options){
  ...
  this.commandDelay = 10000;

  this.newMessageDelay = 1500;
...
sendNextCommand(){
  ...
  if(!this.commandInProgress && this.commandsList.length > 0){
    ...
    else if(command == "ss") command = ESC;  // end input text
    else if(command == "ee") command = ctrlZ;  // end input text
    else command += "\r"

    // Create Timeout:
    if(this.commandTimeout !== undefined) clearTimeout(this.commandTimeout);
    this.commandTimeout = setTimeout(() => this.throwCommand(), this.commandDelay);
...
testBuffer(){
  ...
  for(let i=0; i<data.length; i++){
    ...
    if(d.startsWith("+CMTI")){
      ...
      setTimeout(() => this.emit("new message", idx), this.newMessageDelay);
      continue;
    }
...
// Throw command if no response after <commandDelay> ms:
throwCommand(){
  let answer = "throw: " + this.commandsList[0].command
  this.completeCommand(answer);
}
